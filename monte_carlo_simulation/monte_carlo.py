...
""" Define the corrected payoff function for a digital call option """
...
""" Print results for European call option """
...
""" Print results for Digital call option """
...
""" Calculate and plot the differences for European call option """
differences = []

for N in Ns:
    paths_euler = euler_scheme(S0, sigma, r, T, m, N)
    price_euler = np.exp(-r * T) * np.mean(np.maximum(paths_euler[-1] - K, 0))
    difference = price_euler - bs_call_price
    differences.append(difference)

# Plot the differences between Euler scheme prices and Black-Scholes price
plt.figure()
